...
function setSlider() {
    ...
    if (active === 0) {
        container.style.backgroundImage = 'radial-gradient(circle, #e8700e, #e8700e, #0e98e8, #0e98e8)';
        ...
        container.style.backgroundImage = 'radial-gradient(circle, #0ce9f5, #0ce9f5, #1c1e1f, #1c1e1f)';
        activeDescription.style.color = cor3;
        activeCarInfo.style.color = cor3;
        activeH2.style.color = cor3;
        activeLi.style.backgroundColor = cor2; // Muda a cor de fundo do li ativo
    } else if (active === 2) {
        container.style.backgroundImage = 'radial-gradient(circle, #0e98e8, #eb701e, #eb701e, #1c1e1f)';
        activeDescription.style.color = cor3;
        activeCarInfo.style.color = cor3;
        activeH2.style.color = cor3;
        activeLi.style.backgroundColor = cor2; // Muda a cor de fundo do li ativo
...
document.addEventListener('keydown', (event) => {
    if (event.key === 'ArrowRight') {
        nextButton.click(); // Simula o clique no botão "Next" ao pressionar a seta para a direita
    } else if (event.key === 'ArrowLeft') {
        prevButton.click(); // Simula o clique no botão "Prev" ao pressionar a seta para a esquerda
    }
});
